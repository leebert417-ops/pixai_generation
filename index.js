...
import {
    ...
    eventSource,
    event_types,
} from "../../../../script.js"; // 路径从 ../../../ 变为 ../../../../
import {
    extension_settings,
    renderExtensionTemplateAsync,
    getContext,
} from "../../../extensions.js"; // 路径从 ../../ 变为 ../../../
import {
    getBase64Async,
    humanizedDateTime,
    saveBase64AsFile,
} from "../../../utils.js"; // 路径从 ../../ 变为 ../../../
 ...
 */
async function generatePixaiImage(
    prompt,
    negativePrompt,
    overrides = {},
    signal
    ...
    if (!createResponse.ok) {
        ...
        throw new Error(
            `PixAI 创建任务失败 (HTTP ${createResponse.status}): ${errText}`
        );
    }
    ...
    while (Date.now() - startTime < MAX_WAIT_TIME_MS) {
        ...
        if (!statusResponse.ok) {
            throw new Error(
                `PixAI 状态检查失败 (HTTP ${statusResponse.status})`
            );
        }
        ...
        if (status === "completed") {
            ...
            const errorDetails =
                statusData.error || statusData.errorDetails || "未知错误";
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            callback: async (args, trigger) => {
                ...
                const characterName =
                    context.characters[context.characterId]?.name || "PixAI";
                ...
                try {
                    // 1. 调用 API
                    const result = await generatePixaiImage(
                        prompt,
                        negativePrompt,
                        args,
...
jQuery(async () => {
    ...
    $("#pixai_negative_prompt").on("input", async function () {
        ...
        if (!CSS.supports("field-sizing", "content")) {
            const { resetScrollHeight } = await import("../../../utils.js"); // 路径已修正
